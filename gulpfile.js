...
if (argv.path) {
    config.path.project.path = argv.path;
    config.path.project.assets = argv.path;
    gutil.log('Project path', gutil.colors.magenta(config.path.project.path));
}
if (argv.assets) {
    config.path.project.assets = argv.assets;
    gutil.log('Project assets', gutil.colors.magenta(config.path.project.assets));
}
...
gulp.task('build', [
    ...
    tasks.push(function (callback) {
        ...
        return gulp.src(config.path.tmpDist + 'config/styles.scss')
            .pipe(replace(/\$path: '\/';/g, "$path: '" + config.path.project.assets + "';"))
    ...
    tasks.push(function (callback) {
        ...
        config.webpack.output.publicPath = config.path.project.assets + 'js/';
        config.webpack.entry.main = config.path.tmpDist + "/main.js";
        config.webpack.output.chunkFilename = '[id].[hash].[chunkhash].js';
...
gulp.task('prompt:path', function () {
    ...
    return gulp.src(config.path.assets + 'index.html')
        .pipe(prompt.prompt({
            ...
            default: config.path.project.path
        }, function (res) {
            // Path
            config.path.project.path = res.path;

            // Assets
            if(argv.assets === void(0)) {
                config.path.project.assets = res.path;
            }
        }));
});
...
gulp.task('default', [
    ...
    // Setup
    config.webpack.module.loaders[1].loader = 'style!css?localIdentName=[path][name]---[local]!sass!postcss';
...
gulp.task('build:server', [
    ...
    // Assets
    app.use(config.path.project.path, express.static(config.path.dist));
    ...
    app.listen(config.port.dist, function () {
        gutil.log('Server On http://localhost:' + config.port.dist + config.path.project.path);
        callback();
...
function setupIndex(dest, callback, env) {
    return gulp.src(config.path.assets + 'index.html')
        ...
        .pipe(replace(/\{path\}/g, config.path.project.path))
        .pipe(replace(/\{assets\}/g, config.path.project.assets))
...
function setupModules(callback) {
    ...
    try {
        var modules = _.map(loadJSON(json), function (value, key) {
            return '"' + key + '": function() { return new Promise(function(resolve, reject) { require.ensure(["' + value + '"], (require) => { resolve(require("' + value + '").default); }); }); }';
        }).join(",");
